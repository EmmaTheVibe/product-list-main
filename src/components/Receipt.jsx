...
export default function Receipt({ resetAll, orderItems, colors, mq }) {
  return (
    <div className="receipt">
      ...
      <h1 style={{ marginTop: "12px" }}>Order Confirmed</h1>
      <p style={{ color: colors.deepRose }}>We hope you enjoy your food</p>
      <div className="confirm-list">
        {orderItems.map((orderItem) => (
          <ConfirmItem
            name={
              mq === "split"
                ? orderItem.name.length > 20
        ...
        <Total orderItems={orderItems} className={"sumB"} colors={colors} />
      </div>
      <button onClick={resetAll} className="close-receipt">
        Start New Order
      </button>
    </div>
  );
...
function ConfirmItem({ name, thumbnail, times, rate, total, colors }) {
  return (
    <div className="confirm-item">
      <div className="left">
        ...
        <div className="item-details">
          <p style={{ fontWeight: "600" }} className="item-name">
            {name}
          </p>
